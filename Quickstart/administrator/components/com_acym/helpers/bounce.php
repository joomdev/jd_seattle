...
class acymbounceHelper
{
    ...
    public function handleMessages()
    {
        ...
        while (($msgNB > 0) && ($this->_message = $this->getMessage($msgNB))) {
                ...
                if (!empty($results[0])) {
                    ...
                    foreach ($results[0] as $oneEmail) {
                        if (!preg_match($removeEmails, $oneEmail)) {
                            ...
                            $this->_message->userid = $this->userClass->getUserIdByEmail($this->_message->subemail);
            ...
            if (empty($this->_message->mailid) && !empty($this->_message->userid)) {
                $this->_message->mailid = acym_loadResult('SELECT `mail_id` FROM #__acym_user_stat WHERE `user_id` = '.(int)$this->_message->userid.' ORDER BY `send_date` DESC');
            }
            ...
            foreach ($this->bounceMessages as $mailid => $bouncedata) {
                ...
                if (!empty($bouncedata['userids'])) {
                    ...
                    foreach ($bouncedata['ruletriggered'] as $userid => $rulename) {
                        $valueInsert[] = '('.(int)$mailid.','.(int)$userid.',1,'.acym_escapeDB($rulename).')';
                    }
                ...
                acym_query('UPDATE #__acym_mail_stat SET `bounce_unique` = `bounce_unique` + '.$bouncedata['nbbounces'].$updateBounceDetails.' WHERE `mail_id` = '.intval($mailid).' LIMIT 1');
            }
    ...
    private function _actionuser(&$oneRule)
    {
        ...
        if (in_array('empty_queue_user', $oneRule->action_user)) {
            $affected = acym_query('DELETE FROM #__acym_queue WHERE user_id = '.$this->_message->userid);
