		...
		<div class="grid-x text-center">
			...
			<div class="medium-4 cell grid-x grid-margin-x align-center">
				<div class="medium-shrink cell">
					<button type="button" class="button button-secondary expanded acy_button_submit" data-task="edit" data-step="filter"><?php echo acym_translation('ACYM_NEW_MASS_ACTION'); ?></button>
				</div>
				<div class="medium-shrink cell">
            ...
            <?php } else { ?>
				<div class="grid-x grid-margin-x">
					<div class="large-auto cell">
                        <?php echo acym_filterSearch(htmlspecialchars($data["search"]), 'automation_search', 'ACYM_SEARCH_AUTOMATION'); ?>
					</div>
					...
					<div class="large-shrink medium-6 cell">
						<button data-task="edit" data-step="filter" class="button expanded button-secondary acy_button_submit"><?php echo acym_translation('ACYM_NEW_MASS_ACTION') ?></button>
					</div>
					<div class="large-shrink medium-6 cell">
                ...
                <?php } else { ?>
					...
					<div class="grid-x acym__listing">
                        ...
                        <?php foreach ($data['allAutomations'] as $automation) { ?>
							<div elementid="<?php echo htmlspecialchars($automation->id); ?>" class="grid-x cell acym__listing__row">
								<div class="medium-shrink small-1 cell">
									<input id="checkbox_<?php echo htmlspecialchars($automation->id); ?>" type="checkbox" name="elements_checked[]" value="<?php echo htmlspecialchars($automation->id); ?>">
								</div>
								<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
									<div class="grid-x medium-5 small-9 cell acym__listing__title">
										<a class="cell auto" href="<?php echo acym_completeLink('automation&task=edit&step=info&id=').htmlspecialchars($automation->id); ?>">
                                            <?php echo "<h6>".htmlspecialchars($automation->name)."</h6>"; ?>
										</a>
									</div>
									<div class="medium-auto hide-for-small-only cell grid-x">
										<h6 class="cell acym__listing__text">
                                            <?php echo empty($automation->description) ? '' : (strlen($automation->description) >= 50 ? acym_tooltip(substr($automation->description, 0, 50), $automation->description).'...' : $automation->description); ?>
										</h6>
									</div>
									<div class="xxlarge-2 small-3 cell acym__listing__controls grid-x">
										<div class="text-center cell">
                                            ...
                                            echo '<i table="automation" field="active" elementid="'.htmlspecialchars($automation->id).'" class="acym_toggleable fa '.$class.'"></i>';
                                            ?>
										</div>
									</div>
									<div class="medium-1 hide-for-small-only grid-x">
										<h6 class="cell text-center acym__listing__text"><?php echo htmlspecialchars($automation->id); ?></h6>
									</div>
								</div>
							</div>
                        <?php } ?>
