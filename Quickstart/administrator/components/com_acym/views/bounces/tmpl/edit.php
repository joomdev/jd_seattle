...
?><form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm" data-abide novalidate>
	<input type="hidden" name="bounce[id]" value="<?php echo(empty($data['rule']) ? '' : $data['rule']->id) ?>">
	<input type="hidden" name="bounce[ordering]" value="<?php echo(empty($data['rule']) ? '' : $data['rule']->ordering) ?>">
	<div id="acym__bounces__listing" class="acym__content grid-x cell grid-margin-x">
		...
		<div class="acym__content cell grid-x large-6">
			...
			<label class="cell grid-x">
				...
				<span class="cell medium-8 acym__label grid-x">
					<span style="margin-left: -10px">#</span><input class="intext_input_large medium-11 cell" type="text" name="bounce[regex]" value="<?php echo(!empty($data['rule'] && !empty($data['rule']->regex)) ? $data['rule']->regex : '') ?>"><span class="medium-1 cell">#ims</span>
				</span>
			</label>
			<div class="cell grid-x margin-top-1"><?php echo acym_switch('bounce[active]', (!empty($data['rule']) ? $data['rule']->active : 1), acym_translation('ACYM_ENABLED'), array(), 'medium-4') ?></div>
            <?php
            $valuesRegex = array(
                'senderInfo' => acym_translation('ACYM_SENDER_INFORMATION'),
                'subject' => acym_translation('ACYM_SUBJECT'),
                'body' => acym_translation('ACYM_BODY'),
            );
            acym_checkbox($valuesRegex, 'bounce[executed_on][]', (!empty($data['rule'] && !empty($data['rule']->executed_on)) ? $data['rule']->executed_on : array()), acym_translation('ACYM_EXECUTE_REGEX_ON'), 'cell margin-top-1 grid-x', 'medium-4 margin-right-1'); ?>
			<div class="cell grid-x margin-top-1"><?php echo acym_switch('bounce[increment_stats]', (!empty($data['rule']) ? $data['rule']->increment_stats : 1), acym_translation('ACYM_INCREMENT_BOUNCE_STATISTICS_IF_RULE_MATCHES'), array(), 'medium-4') ?></div>
		</div>
		<div class="acym__content cell grid-x large-6">
			<div class="cell">
				<h1 class="acym__title__light__blue"><?php echo acym_translation('ACYM_ACTION_ON_USER') ?></h1>
				<div class="cell grid-x">
					<p class="acym__label"><?php echo acym_translation_sprintf('ACYM_EXECUTE_ACTIONS_AFTER', '<input type="text" name="bounce[execute_action_after]" value="'.(!empty($data["rule"]) ? $data["rule"]->execute_action_after : "0").'" class="intext_input">') ?></p>
                    <?php
                    $valuesActionUser = array(
                    ...
                    acym_checkbox($valuesActionUser, 'bounce[action_user][]', (!empty($data['rule'] && $data['rule']->action_user) ? $data['rule']->action_user : array()));
                    echo '</div>'; ?>
				</div>
			</div>
			<div class="cell">
				...
				<div class="cell grid-x">
                    ...
                    acym_checkbox($valuesActionEmail, 'bounce[action_message][]', (!empty($data['rule'] && $data['rule']->action_message) ? $data['rule']->action_message : array()));
                    echo '</div>';
                    ?>
				</div>
			</div>
