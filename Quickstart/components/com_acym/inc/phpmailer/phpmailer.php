...
class acyPHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new acyException($error_message);
            }
    ...
    public function preSend()
    {
        ...
        if (ini_get('mail.add_x_header') == 1
            ...
            trigger_error(
                'Your version of PHP is affected by a bug that may result in corrupted messages. To fix it, upgrade your PHP version to 7.0.17+ or 7.1.3+. ' .
                'You could also switch to sending using SMTP or disable the mail.add_x_header option in your php.ini as a workaround.',
                E_USER_WARNING
            );
        }
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new acyException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new acyException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
            ...
            if (!empty($this->DKIM_domain)
                ...
                $this->MIMEHeader = static::normalizeBreaks($header_dkim) . $this->MIMEHeader;
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new acyException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new acyException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    {
		require_once dirname(__FILE__).DS.'smtp.php';
        $bad_rcpt = array();
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new acyException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new acyException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new acyException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
			...
			$this->setError($errorLbl . ' (' . $errstr . ') ');
			// Avoid the nested MAIL command error
			$this->smtp->Reset();
            throw new acyException($this->ErrorInfo);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw new acyException($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new acyException($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
							...
							throw new acyException($errorLbl);
                        }
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new acyException($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new acyException($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new acyException($this->lang('signing') . openssl_error_string());
                }
            } catch (acyException $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new acyException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!file_exists($path)) {
                throw new acyException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
