...
?><form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm">
	<div id="acym__list__subscribers" class="acym__content">
        ...
        <?php } else { ?>
			...
			<div class="grid-x">
                <?php
                $actions = ['unsubscribeUsers' => acym_translation('ACYM_UNSUBSCRIBE')];
			...
			<div class="grid-x acym__listing">
                ...
                <?php foreach ($data['listSubscribers'] as $oneSubscriber) { ?>
					<div class="grid-x cell acym__listing__row" elementid="<?php echo acym_escape($oneSubscriber->id); ?>">
						<div class="medium-shrink small-1 cell">
							<input id="checkbox_<?php echo acym_escape($oneSubscriber->id); ?>" type="checkbox" name="elements_checked[]"
								   value="<?php echo acym_escape($oneSubscriber->id); ?>">
						</div>
						<div class="cell grid-x medium-auto small-11 margin-left-2">
							...
							<div class="cell auto text-center">
                                <?php
                                echo acym_tooltip('<i class="fa fa-user-times acym__list__subscribers__unsubscribe_one" userId="'.acym_escape($oneSubscriber->id).'"></i>', acym_translation('ACYM_UNSUBSCRIBE'));
                                ?>
							</div>
						</div>
					</div>
