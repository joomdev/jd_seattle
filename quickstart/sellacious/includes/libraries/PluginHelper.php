...
abstract class JPluginHelper
{
	...
	public static function importPlugin($type, $plugin = null, $autocreate = true, \JEventDispatcher $dispatcher = null)
	{
		...
		if (!$defaults || !isset($loaded[$dispatcherHash][$type]))
		{
			...
			{
				if (static::validate($plugins[$i]) && $plugins[$i]->type === $type && ($plugin === null || $plugins[$i]->name === $plugin))
				{
	...
	/**
	 * Check whether we can load this plugin
	 *
	 * @param   object  $plugin  The plugin record to be checked against the whitelist
	 *
	 * @return  bool
	 *
	 * @since   1.6.0
	 */
	protected static function validate($plugin)
	{
		$allowed = array(
			'authentication'     => 'cookie,gmail,joomla,ldap',
			'captcha'            => 'recaptcha',
			'content'            => 'contact,emailcloak,fields,finder,joomla,loadmodule,pagebreak,pagenavigation,vote',
			'editors'            => 'codemirror,none,tinymce',
			'editors-xtd'        => 'article,contact,fields,image,menu,module,pagebreak,readmore',
			'system'             => 'cache,debug,fields,highlight,log,logout,p3p,redirect,remember,sef,sessiongc',
			'twofactorauth'      => 'totp,yubikey',
			'user'               => 'contactcreator,joomla,profile',
			'sellacious'         => null,
			'sellaciousrules'    => null,
			'sellaciousshipment' => null,
			'sellaciouspayment'  => null,
		);

		if (array_key_exists(strtolower($plugin->type), $allowed))
		{
			$names = $allowed[strtolower($plugin->type)];

			if ($names === null || in_array($plugin->name, explode(',', $names)))
			{
				return true;
			}

			if (false !== strpos($plugin->name, 'sellacious'))
			{
				return true;
			}
		}
