...
class LoginModelLogin extends JModelLegacy
{
	...
	protected function populateState()
	{
		...
		$credentials = array(
			...
			'password'  => $app->input->post->get('passwd', '', 'RAW'),
			'secretkey' => $app->input->post->get('secretkey', '', 'RAW'),
		);
	...
	protected static function _load($module)
	{
		...
		if (!($clean = $cache->get($cacheid)))
		{
			...
			$query = $db->getQuery(true)
				...
				->from('#__modules AS m')
				->where('m.module =' . $db->quote($module) . ' AND m.client_id = ' . $db->q($clientId))
				->join('LEFT', '#__extensions AS e ON e.element = m.module AND e.client_id = m.client_id')
				->where('e.enabled = 1');

			// Filter by language
			if ($app->getName() == 'sellacious' && $app->getLanguageFilter())
			{
