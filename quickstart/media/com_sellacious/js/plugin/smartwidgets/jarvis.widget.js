...
(function ($, window, document, undefined) {
	...
	function Plugin(element, options) {
		...
		this.toggleLabel = this.o.toggleLabel.split('|');
				...
				if (!thisHeader.parent()
					...
					if (self.o.toggleButton === true && tWidget.data('widget-togglebutton') ===
						...
						if (tWidget.data('widget-collapsed') === true || tWidget.hasClass(
							...
							var toggleLabel = self.toggleLabel[1];
						} else {
							var toggleSettings = self.toggleClass[0];
							var toggleLabel = self.toggleLabel[0];
						}
						var toggleBtn =
							'<a href="#" class="button-icon jarviswidget-toggle-btn" rel="tooltip" title="' + toggleLabel + '" data-placement="bottom"><i class="' +
							toggleSettings + '"></i></a>';
					} else {
						toggleBtn = '';
					}
		...
		_clickEvents : function () {
			...
			self.widget.on(clickEvent, '.jarviswidget-toggle-btn', function (e) {
				...
				if (pWidget.hasClass('jarviswidget-collapsed')) {
					...
					tWidget.tooltip('destroy').attr('title', self.toggleLabel[0]).tooltip()
				} else {
					tWidget.children()
					...
					tWidget.tooltip('destroy').attr('title', self.toggleLabel[1]).tooltip()
				}
			...
			self.widget.on(clickEvent, '.jarviswidget-delete-btn', function (e) {
				 ...
				 **/
				$.SmartMessageBox({
					title : "<i class='"+ self.o.iconDelete +"'></i> " + self.o.labelDelete,
					content : self.o.bodyDelete.replace('%s', ' "' + $.trim(widTitle) + '" '),
					buttons : '[No][Yes]'
